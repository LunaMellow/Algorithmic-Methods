...
bool erBST(Node* node) {
    if (!node)  return true;     //  Tomt - "er da pr.def" binært søketre.
                                 //  En node til VENSTRE er større eller lik:
    if (node->left  &&  finnMax(node->left) >= node->ID)  return false;
                                 //  En node til HØYRE er mindre:
...
int finnMax(Node* node) {
    ...
    int max = node->ID,            //  Initierer til nodens egen ID.
        lmax = finnMax(node->left),    //  Finner venstre og høyre subtrærs
        rmax = finnMax(node->right);   //    største verdier.
...
int finnMin(Node* node) {
    if (!node)  return MAX;        //  Treet er tomt - returnerer 'høy' verdi.
    int min = node->ID,            //  Initierer til nodens egen ID.
        lmin = finnMin(node->left),    //  Finner venstre og høyre subtrærs
